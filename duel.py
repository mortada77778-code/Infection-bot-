...
def add_win(user_id, username):
    ...
    db[uid]["wins"] += 1
    db[uid]["name"] = username  # تحديث الاسم لو تغير
    save_leaderboard(db)

# قاعدة بيانات التعويذات
SPELLS_DATABASE = {
...
FLAVOR_MESSAGES = {
    "attack_hit": [
        ...
        "ارتجّت جدران قاعة هوجوورتس من شدة العصف السحري!",
        "اندفعت طاقة نارية مرعبة نحو الهدف دون أن يمتلك فرصة للهروب!",
        "تطاير الغبار من الحجارة العتيقة إثر صدمة التعويذة المباشرة!"
    ],
    "shield_up": [
...
class DuelSession:
    ...
    async def execute_round(self, interaction: discord.Interaction):
        ...
        active_flavor = random.choice(flavor_picks) if flavor_picks else "تردد صدى التعويذات في أرجاء القاعة الحجرية."
...
class DuelCog(commands.Cog):
    ...
    async def duel_command(self, ctx, member: discord.Member = None):
        if not member:
            await ctx.send(f"⚠️ يرجى عمل منشن للساحر المراد مبارزته: `!مبارزة @اسم_الشخص`", delete_after=10)
            return
    ...
    @commands.command(name="صدارة")
    async def leaderboard_command(self, ctx):
        db = load_leaderboard()
        if not db:
            await ctx.send("⚠️ **لا توجد أي انتصارات مسجلة في السجل حتى الآن! ابدأوا المبارزات لتصنعوا التاريخ.**")
            return

        # ترتيب اللاعبين تنازلياً حسب عدد الانتصارات
        sorted_players = sorted(db.values(), key=lambda x: x["wins"], reverse=True)[:10]

        desc = "🏆 **قائمة أبطال حلبة هوجوورتس السحرية:**\n\n"
        for idx, player in enumerate(sorted_players, 1):
            medal = "🥇" if idx == 1 else ("🥈" if idx == 2 else ("🥉" if idx == 3 else f"#{idx}"))
            desc += f"{medal} **{player['name']}** ── 🎯 `{player['wins']}` انتصار\n"

        embed = discord.Embed(
            title="📜 لوحة صدارة مبارزات العصي",
            description=desc,
            color=0xd4af37
        )
        embed.set_footer(text=AUTHOR_SIGNATURE)
